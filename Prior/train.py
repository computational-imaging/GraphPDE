...
def train(model, train_dataloader, epochs, lr, steps_til_summary, epochs_til_checkpoint, model_dir,
          ...
          params=None,lr_latent=None,wandb=None):

    if lr_latent==None:
        optim = torch.optim.Adam(lr=lr, params=model.parameters())
    else:
        optim = torch.optim.Adam([{"params":model.decoder.parameters(),'lr':lr},
        ...
        for epoch in range(epochs):
            if not epoch % epochs_til_checkpoint and epoch:
                torch.save({'model_state_dict': model.state_dict(),
                            'mean': model.mean,
                            'varience': model.varience,
                            'latent_vec': model.lat_vecs.weight.data.detach(),
            ...
            for step, (model_input, gt) in enumerate(train_dataloader):
                ...
                if not total_steps % steps_til_summary:
                    ...
                    summary_fn(model, model_input, gt, model_output, writer, total_steps,'train',wandb)
